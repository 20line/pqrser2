...
def add_confirm_text(name: str, url: str, price_ceiling: int | None, stop_words: list[str], freq_hint: str) -> str:
    lines = [
        ...
        f"Стоп-слова: {', '.join(stop_words) if stop_words else 'нет'}",
        "",
        freq_hint,
    ]
    return "\n".join(lines)
...
def history_text(profile_name: str, rows: Sequence) -> str:
    ...
    for r in rows:
        ...
        lines.append(f"{icon} {r['title']} — {format_price(r['price'])}\n{r['url']}")
...
def stats_text(
    ...
    for r in rows:
        ...
        lines.append(
            f"• {r['name']}: сегодня найдено {r['found_today']}, "
